let like = document.getElementById('mainlikehover')
if(mynickname == null){
    like.addEventListener('click', nologinsortlike)
}else{
    like.addEventListener('click', sortlike)
}
console.log("확인작업 : " + mynickname)
function nologinsortlike(){
    alert("로그인 후 이용가능 합니다.")
}
async function sortlike(e) {
    ...
    let sortlikeUrl = "/sortlikeAjax?"
    if (contextPath != null) {
        sortlikeUrl = contextPath + sortlikeUrl
    }
    let option = {method: "post"}
    try {
        let res = await fetch(sortlikeUrl, option);
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            let likecount =
                "<span style='margin-left: 15px;' class='likelistcount'>" + result[i].likeListCount + "</span>"
            let likeheart =
        ...
        let sortareadiv = "<h5 class='mainpopularhover' id='mainpopularhover'>" + '인기' + "</h5>" + "&nbsp;|&nbsp;" +
            "<h5 class='mainlikehover' id='mainlikehover' style='font-weight: bold'>" + '찜' + "</h5>" + "&nbsp;|&nbsp;" +
        ...
        console.log("sortlike fetch 후 err : " + err)
    }
}