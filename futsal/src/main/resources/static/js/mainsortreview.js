let reviewsortbtn = document.getElementById('mainreviewhover')
reviewsortbtn.addEventListener('click', sortreview)

async function sortreview(e) {
    ...
    let sortreviewUrl = "/sortreviewAjax?"
    if (contextPath != null) {
        sortreviewUrl = contextPath + sortreviewUrl
    }
    let option = {method: "post"}
    try {
        let res = await fetch(sortreviewUrl, option);
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            let likecount ="";
            if (result[i].likeListCount == undefined || result[i].likeListCount == 0 || result[i].likeListCount == null) {
        ...
        let sortareadiv = "<h5 class='mainpopularhover' id='mainpopularhover'>" + '인기' + "</h5>" + "&nbsp;|&nbsp;" +
            ...
            "<h5 class='mainreviewhover' id='mainreviewhover' style='font-weight: bold'>" + '리뷰' + "</h5>"
        ...
        console.log("sortreview fetch 후 err : " + err)
    }
}